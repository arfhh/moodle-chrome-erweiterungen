/* Moodle Notenstufen Autofill — Einstellungsseite, aus popup.js v2.7.0 uebernommen
 * (WXT-Umzug 17.09.2026). Dieselbe Datei bedient sowohl den Popup- als auch den
 * Options-Entrypoint (siehe entrypoints/popup und entrypoints/options) — beide
 * rufen nur initNotenstufenEinstellungen() auf, damit es nur eine Fassung der
 * Tabellen-/Speicherlogik gibt.
 * Aenderungen beim Umzug: chrome.* -> browser.*, IIFE -> export function.
 *
 * A. Spielhoff · CC BY-SA 4.0
 */
import { browser } from 'wxt/browser';

export function initNotenstufenEinstellungen() {
  'use strict';
...
function speichern(noten) {
    browser.storage.local.set({ notenstufen: noten }, () => {
